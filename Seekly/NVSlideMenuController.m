...
- (id)initWithMenuViewController:(UIViewController *)menuViewController andContentViewController:(UIViewController *)contentViewController
{
	...
	if (self)
	{
		...
		self.showShadowOnContentView = NO;
	}
...
- (void)viewDidLoad
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(toggleMenuAnimated:) name:@"toggleMenuAnimated" object:nil];
}
...
- (void)panGestureTriggered:(UIPanGestureRecognizer *)panGesture
{
    ...
    if([[NSUserDefaults standardUserDefaults]valueForKey:@"slideend"]==NO)
    {
