...
// Constants
static CGFloat const JDFPeekabooCoordinatorNavigationBarHorizontalHeightDifferential = 15.0f; // This is the difference between the size of the navigation bar when it is in portrait and landscape. This should probably be handled more elegantly.
...
{
     [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(fullyExpandViews) name:@"HideView" object:nil];
    if (!_containingView) {
        return _scrollView;
    }
...
- (instancetype)init
{
    self = [super init];
    if (self) {
        self.topViewMinimisedHeight = 15.0f;
